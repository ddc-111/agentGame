...
def run_features():
    ...
    # 执行add_feature类型的任务（所有优先级）
    feature_tasks = [t for t in tasks if t.get('type') == 'add_feature']
    
    # 限制数量到15个
    tasks_to_execute = feature_tasks[:15]
...
def load_tasks():
    ...
    # 找到包含add_feature任务的文件
    for task_file in task_files[:5]:  # 检查最近5个文件
        with open(task_file, 'r', encoding='utf-8') as f:
            data = json.load(f)
        tasks = data.get('tasks', [])
        feature_tasks = [t for t in tasks if t.get('type') == 'add_feature']
        if feature_tasks:
            print(f"加载任务文件: {task_file.name}")
            return tasks
...
"""
    ...
    # 保存报告
    report_file = REPORTS_DIR / f"features_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
